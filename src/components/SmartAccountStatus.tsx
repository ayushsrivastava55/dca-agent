...
export default function SmartAccountStatus() {
  ...
  if (loading) return <div className="text-xs text-black">Setting up smart account…</div>;
  if (error) return <div className="text-xs text-[var(--color-error)]">{error}</div>;
  if (!ready) return null;

  return (
    <div className="text-xs rounded-full border border-black/20 px-3 py-1 text-black bg-blue-50">
